...
**logging** | **Number** | Number of log lines sent (alias for `log`). | [optional] 
...
**otfp_shield** | **Number** | Number of responses that came from the Fastly On-the-Fly Packaging service for video-on-demand via a shield. | [optional] 
...
**hash_sub_time** | **Number** | Time spent inside the `vcl_hash` Varnish subroutine (in nanoseconds). | [optional] 
